    ...
    def _parse_statement(self, statement: _ast.AST) -> None: # pylint: disable=no-member
        ...
        if statement.ast_type == _ast.ASTType.Rule: # pylint: disable=no-member
            for rule in self._parse_rule(statement):
                self.callback(rule)
        elif statement.ast_type == _ast.ASTType.Program: # pylint: disable=no-member
            for statement in self._parse_program_statement(statement):
                self.callback(statement)
        elif statement.ast_type == _ast.ASTType.ShowSignature: # pylint: disable=no-member
            for stm in self._parse_show_signature_statement(statement):
                self.callback(stm)
        elif statement.ast_type == _ast.ASTType.ShowTerm: # pylint: disable=no-member
