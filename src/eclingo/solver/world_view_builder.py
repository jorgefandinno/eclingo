...
class WorldWiewBuilder:
    def __init__(self, control: internal_control.InternalStateControl):
        self._epistemic_show_pos_mapping: Dict[
            Symbol, EpistemicLiteral
        ] = control.show_mapping.positive
        self._epistemic_show_neg_mapping: Dict[
            Symbol, EpistemicLiteral
        ] = control.show_mapping.negative
...
class WorldWiewBuilderWithShow(WorldWiewBuilder):
    ...
    def _add_rules_for_negative_check(
        self,
        control_show: internal_control.InternalStateControl,
        symbolic_show_atoms: Iterator[SymbolicAtom],
    ) -> None:
        """
        ...
        with control_show.symbolic_backend() as backend:
            for symbolic_atom in symbolic_show_atoms:
                if not symbolic_atom.is_fact:
                    ...
                    backend.add_rule(
                        [symbolic_atom_not], [], [symbolic_atom.symbol], False
                    )
    ...
    def world_view_from_candidate(self, candidate: Candidate):
        ...
        with self.control.solve(
            ...
            for symbol in itertools.chain(
                self._epistemic_show_pos_mapping, self._epistemic_show_neg_mapping
            ):
