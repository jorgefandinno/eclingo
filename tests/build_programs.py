...
from programs_helper import Program

from eclingo.solver.candidate import Candidate


class ProgramStr(NamedTuple):
    program: str
    non_ground_reification: str = None
    candidates: str = None
    wv_candidates: str = None
    unoptimiced_candidates: str = None
...
def build_candidates(candidate: Iterable[str]) -> List[Candidate]:
    return [build_candidate(c) for c in candidate]


def complete_program(program: ProgramStr) -> Program:
    candidates = program.candidates
    if program.candidates is not None:
    ...
    return Program(
        ...
        unoptimiced_candidates=program.unoptimiced_candidates,
        candidates=candidates,
        wv_candidates=program.wv_candidates,
        non_ground_reification=program.non_ground_reification,
...
str_programs = ",\n".join(pp.pformat(complete_program(program)) for program in programs)
