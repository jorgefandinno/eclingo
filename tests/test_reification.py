...
def parse_program(stm, parameters=[], name="base"):
    ret = []
    parser.parse_program(stm, ret.append, parameters, name, config = AppConfig(semantics="c19-1", verbose=0, use_reification=True))
    return flatten(ret)
...
class Test(TestCase):
    ...
    def test_non_epistemic_rules(self):
        self.assert_equal_program(parse_program("a :- b, c, not d, not not e."), "u(a) :- u(b), u(c), not u(d), not not u(e).")
        self.assert_equal_program(parse_program("-a :- b, -c, not -d, not not -e."), "u(-a) :- u(b), u(-c), not u(-d), not not u(-e).")
    ...
    def test_epistemic_atom_with_default_negation(self):
        self.assert_equal_program(parse_program(":- &k{ not a}."), ":- k_not_u(a). not_u(a) :- not u(a). {k_not_u(a)} :- not_u(a).")
        self.assert_equal_program(parse_program("b :- &k{ not a}."), "u(b) :- k_not_u(a). not_u(a) :- not u(a). {k_not_u(a)} :- not_u(a).")
        self.assert_equal_program(parse_program(":- &k{ not not a}."), ":- k_not2_u(a). not2_u(a) :- not not u(a). {k_not2_u(a)} :- not2_u(a).")
...
# Not changed below
    ...
    def test_epistemic_with_variables(self):
        ...
        self.assert_equal_program(parse_program(":- &k{-a(V0)}."), ":- k_u(sn_a(V0)). sn_a(V0) :- -a(V0). {k_u(sn_a(V0))} :- u(sn_a(V0)).")
        '''
        self.assert_equal_program(parse_program(":- &k{- -a(V0)}."), ":- k_u(a(V0)). {k_u(a(V0))} :- u(a(V0)).")
    ...
    def test_parameters01(self):
        self.assert_equal_program(parse_program("a(1..n).", ["n"], "parametrized" ), "#program parametrized(n). u(a(1..n)).")

    def test_parameters02(self):
        self.assert_equal_program(parse_program("a(1..n).", ["n"], "base" ), "#program base(n). u(a(1..n)).")

    def test_heuristic(self):
        self.assert_equal_program(parse_program("#heuristic a. [1,sign]", [], "base" ), "#heuristic u(a). [1,sign]")
